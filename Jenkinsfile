...
pipeline {
	...
	stages {
		...
		stage('Unit and Device tests') {
			steps {
				sh "rm -rf ${env.GRADLE_PROJECT_MODULE_NAME}/temp-results"
				sh "mkdir ${env.GRADLE_PROJECT_MODULE_NAME}/temp-results"
				...
				sh "mv ${env.GRADLE_PROJECT_MODULE_NAME}/build ${env.GRADLE_PROJECT_MODULE_NAME}/temp-results/build-unittest"

				// Run device tests for package: org.catrobat.catroid.test
				sh "./buildScripts/build_step_run_tests_on_emulator__test_pkg"
				// ensure that the following test run does not overwrite the results
				sh "mv ${env.GRADLE_PROJECT_MODULE_NAME}/build ${env.GRADLE_PROJECT_MODULE_NAME}/temp-results/build-test-test-pkg"
